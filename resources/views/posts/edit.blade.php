<x-app-layout>
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-white shadow-md">
        <h2 class="text-center text-lg font-bold pt-6 tracking-widest">出品編集</h2>
        ...
        <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="category"
                    ...
                    required value="{{ old('category', $post->category) }}">
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="title"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full py-2 px-3"
                    required placeholder="例) 除草作業を行います" value="{{ old('title',$post->title) }}">
            </div>
            <div class="mb-4">
                ...
                <textarea name="body" rows="10"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full py-2 px-3"
                    required placeholder="サービスの内容、ユーザーのメリット、アピールしたい実績など具体的に書いてください">{{ old('body', $post->body) }}</textarea>
            </div>
            <div class="mb-4">
